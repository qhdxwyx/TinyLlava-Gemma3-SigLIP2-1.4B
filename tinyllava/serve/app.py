...
def add_text(state, text, image, image_process_mode):
    ...
    state.add_message(text.strip(), None)
    state.skip_next = False
    return (state, state.to_gradio_chatbot(), "", None)
...
def resolve_max_new_tokens(requested_max_new_tokens, max_context_length, input_ids, num_image_tokens):
    available_tokens = max_context_length - input_ids.shape[-1] - num_image_tokens
    if requested_max_new_tokens is None or int(requested_max_new_tokens) <= 0:
        return available_tokens
    return min(int(requested_max_new_tokens), 1024, available_tokens)
...
def get_response(params):
    ...
    if images is not None and len(images) > 0:
        if len(images) > 0:
            ...
            image = image.unsqueeze(0).to(model.device, dtype=image_dtype)
            ...
            with gr.Column(scale=5):
                ...
                with gr.Accordion("Parameters", open=False) as _:
                    ...
                    max_output_tokens = gr.Slider(
                        ...
                        step=64,
                        interactive=True,
                        label="Max output tokens (0 = auto)",
                    )
