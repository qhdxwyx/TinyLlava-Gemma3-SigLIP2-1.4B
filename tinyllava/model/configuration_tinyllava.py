...
def _resolve_local_third_party_path(model_path):
    if not model_path:
        return model_path
    ...
    if "/third_party/" not in normalized:
        return model_path

    relative = normalized.split("/third_party/", 1)[1]
    project_root = Path(__file__).resolve().parents[3]
    local_candidate = project_root / "third_party" / Path(*relative.split("/"))
...
class TinyLlavaConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.llm_model_name_or_path = _resolve_local_third_party_path(llm_model_name_or_path)
        self.tokenizer_name_or_path = _resolve_local_third_party_path(
            tokenizer_name_or_path or self.llm_model_name_or_path
        )
        self.vision_model_name_or_path = _resolve_local_third_party_path(vision_model_name_or_path)
        self.vision_model_name_or_path2 = _resolve_local_third_party_path(vision_model_name_or_path2)
    ...
    def load_from_config(self, config):
        self.llm_model_name_or_path = _resolve_local_third_party_path(
            getattr(config, 'model_name_or_path',  '')
        )
        self.tokenizer_name_or_path = _resolve_local_third_party_path(
            getattr(config, 'tokenizer_name_or_path', None) or self.llm_model_name_or_path
        )
        self.vision_model_name_or_path = _resolve_local_third_party_path(
            getattr(config, 'vision_tower',  '')
        )
        self.vision_model_name_or_path2 = _resolve_local_third_party_path(
            getattr(config, 'vision_tower2',  '')
        )
    ...
    def _load_text_config(self, text_config=None):
        if self.llm_model_name_or_path is None or self.llm_model_name_or_path == '':
            ...
            if not hasattr(self.text_config, 'hidden_act'):
                if 'gemma' in self.llm_model_name_or_path.lower():
    ...
    def _load_vision_config(self, vision_config=None):
        if self.vision_model_name_or_path is None or self.vision_model_name_or_path == '':
            ...
            self.vision_config = AutoConfig.from_pretrained(self.vision_model_name_or_path)
        ...
        self.vision_config.model_name_or_path = self.vision_model_name_or_path
        self.vision_config.model_name_or_path2 = self.vision_model_name_or_path2
